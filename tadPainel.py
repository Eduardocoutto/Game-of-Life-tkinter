...
def cria(x,y,largura,altura,largq,altq,espaco,corOff,corON,corBg):
    matObjeto = tadMatriz.cria(largura,altura)
    return [x,y,largura,altura,largq,altq,espaco,corOff,corON,corBg,matObjeto]
...
def onoff(linha,coluna,tadPainel,window,ligado,cor=None):
    ...
    if tadMatriz.getElem(tadPainel[10],linha,coluna) != 0:
        window.delete(tadMatriz.getElem(tadPainel[10],linha,coluna))

    r = window.create_rectangle(inicioX, inicioY, fimX, fimY, width=0, fill=cor)
    tadPainel[10] = tadMatriz.setElem(tadPainel[10],linha,coluna,r)
